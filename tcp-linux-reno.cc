...
int main (int argc, char *argv[])
{
  ...
  pointToPointLeaf1.SetChannelAttribute   ("Delay", StringValue ("1ms"));
  ...
  leftToRouter.push_back (pointToPointLeaf1.Install (leftNodes.Get (0), routers.Get (0)));
  leftToRouter.push_back (pointToPointLeaf2.Install (leftNodes.Get (1), routers.Get (0)));
  routerToRight.push_back (pointToPointLeaf1.Install (routers.Get (1), rightNodes.Get (0)));
  routerToRight.push_back (pointToPointLeaf2.Install (routers.Get (1), rightNodes.Get (0)));
  ...
  std::vector <Ipv4InterfaceContainer> leftToRouterIPAddress1;
  std::vector <Ipv4InterfaceContainer> leftToRouterIPAddress2;
  leftToRouterIPAddress1.push_back (ipAddresses.Assign (leftToRouter [0]));
  ipAddresses.NewNetwork ();
  
  leftToRouterIPAddress2.push_back (ipAddresses.Assign (leftToRouter [1]));
  ipAddresses.NewNetwork ();
  ...
  InstallBulkSend (leftNodes.Get (1), routerToRightIPAddress [0].GetAddress (1), port, socketFactory, 2, 0, MakeCallback (&CwndChange));

  // Enable PCAP on all the point to point interfaces
  pointToPointLeaf1.EnablePcapAll (dir + "pcap/ns-3", true);
  pointToPointLeaf2.EnablePcapAll (dir + "pcap/ns-3", true);
